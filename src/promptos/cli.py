...
def run_config(args: argparse.Namespace) -> int:
    ...
    values = {
        ...
        "signals": str(config.signal_spec), "prompt": config.optimization.initial_prompt, "task": config.name,
...
def validate_config(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"task": config.name, "dataset": str(config.dataset.path), "signals": str(config.signal_spec),
                      "runs_dir": str(config.runs_dir), "model": config.models.task_model, "credentials": "environment-only"}, ensure_ascii=False, indent=2))
    return 0
...
def main() -> int:
    ...
    config_run.add_argument("--config", required=True); config_run.add_argument("--prompt", help="Optional one-run prompt override"); config_run.add_argument("--model", help="Optional one-run task model override"); config_run.set_defaults(func=run_config)
    run = sub.add_parser("optimize", help="Run a local, budget-bounded optimization experiment")
